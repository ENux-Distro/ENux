...
This document explains how to install ENux Netinst step-by-step.
...
You will need a USB drive [minimum: 1GB] [recommended 2GB+].
...
```sudo dd if=ENux-Netinst-<version>.iso of=/dev/sdX bs=4M status=progress oflag=sync```
...
- Boot into the ENux Netinst environment

## About the Installer

On ENux Netinst, there's only a single installer, and that's the TUI form of the ENux Installer. Why, because its:
...
When you first boot, the ENux Installer will automatically be ran. It'll do a do a quick internet check to see if you have proper internet connection.
